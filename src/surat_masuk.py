...
class PaddedItemDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        # Ambil opsi style default
        opt = QStyleOptionViewItem(option)
        self.initStyleOption(opt, index)
        
        # Simpan rect asli untuk background seleksi
        original_rect = list(opt.rect.getRect()) # x, y, w, h
        
        # Gambar background seleksi/hover menggunakan rect asli (Full Kotak)
        style = opt.widget.style()
        style.drawPrimitive(style.PrimitiveElement.PE_PanelItemViewItem, opt, painter, opt.widget)
        
        # Manipulasi rect untuk Text (Memberi Padding)
        # adjust(kiri, atas, kanan, bawah) -> Menambah jarak dari tepi
        opt.rect.adjust(10, 5, -10, -5) 
        
        # Hapus state 'Selected' agar text tidak digambar ulang dengan background seleksi (double draw)
        # Kita hanya ingin menggambar Text-nya saja di area yang sudah dikecilkan
        opt.state &= ~style.StateFlag.State_Selected
        opt.state &= ~style.StateFlag.State_HasFocus
        
        # Gambar Text/Konten di area yang sudah ada padding-nya
    ...
    def aksi_tambah(self):
        ...
        if dialog.exec():
            ...
            try:
                ...
                db = connect_db()
                cursor = db.cursor()
                cursor.execute("INSERT INTO surat (nomor_surat, judul_surat, asal_surat, kategori, tanggal, tanggal_surat, keterangan, file_path) VALUES (?, ?, ?, 'masuk', ?, ?, ?, ?)", 
        ...
        if dialog.exec():
            try:
                ...
                if path_baru_input != path_lama:
                    ...
                    final_path = os.path.join(up_dir, f"IN_EDIT_{datetime.now().strftime('%Y%m%d_%H%M%S')}{ext}")
